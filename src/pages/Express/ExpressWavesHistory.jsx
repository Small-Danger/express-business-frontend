...
const ExpressWavesHistory = () => {
  const navigate = useNavigate();
  const [waves, setWaves] = useState([]);
  const [loading, setLoading] = useState(true);
  const [viewMode, setViewMode] = useState('cards'); // 'cards' ou 'table'

  // Charger les vagues clôturées
  useEffect(() => {
    loadWaves();
  }, []);
  ...
  const loadWaves = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        // Filtrer uniquement les vagues clôturées
        const closedWaves = allWaves.filter(wave => wave.status === 'closed');
        setWaves(closedWaves);
      }
    } catch (error) {
      console.error('Erreur lors du chargement des vagues clôturées:', error);
      setWaves([]);
    } finally {
      setLoading(false);
    }
  ...
  const getTripsByStatus = (wave) => {
    const trips = wave.trips || [];
    return {
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-gradient-to-r from-purple-600 to-pink-600 rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-lg">
        <div>
          <Link to="/express/waves" className="text-sm text-purple-100 hover:text-white mb-2 inline-block font-medium">
            ← Retour aux vagues
          </Link>
          <h1 className="text-2xl sm:text-3xl font-extrabold text-white">Historique des Vagues Clôturées</h1>
        </div>
      ...
      {/* Message si aucune vague clôturée */}
      {!loading && waves.length === 0 && (
        <div className="bg-white rounded-xl shadow-lg p-8 sm:p-12 text-center border border-gray-100">
          ...
          <p className="text-lg sm:text-xl font-bold text-gray-900 mb-2">Aucune vague clôturée</p>
          <p className="text-sm text-gray-500">Il n'y a pas encore de vagues clôturées dans le système.</p>
        </div>
      )}
      ...
      {viewMode === 'cards' && waves.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {waves.map((wave) => {
            const tripsByStatus = getTripsByStatus(wave);
            return (
                  ...
                  <div className="pt-3 border-t border-gray-200">
                    <div className="grid grid-cols-2 gap-3 text-sm">
                      <div>
                        ...
                        <p className="font-semibold text-gray-900">{wave.trips_count || 0}</p>
                      </div>
                      <div>
                        <p className="text-gray-600">Frais</p>
                        <p className="font-semibold text-gray-900">{wave.costs_count || 0}</p>
                      </div>
                    </div>
                    ...
                    {wave.trips_count > 0 && (
                      <div className="mt-3 pt-3 border-t border-gray-200">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {tripsByStatus.planned > 0 && (
                            <span className="px-2 py-1 text-xs bg-gray-100 text-gray-700 rounded">
                              {tripsByStatus.planned} Planifié{tripsByStatus.planned > 1 ? 's' : ''}
                            </span>
                          )}
                          {tripsByStatus.in_transit > 0 && (
                            <span className="px-2 py-1 text-xs bg-purple-100 text-purple-700 rounded">
                              {tripsByStatus.in_transit} En transit
                            </span>
                          )}
                          {tripsByStatus.arrived > 0 && (
                            <span className="px-2 py-1 text-xs bg-green-100 text-green-700 rounded">
                              {tripsByStatus.arrived} Arrivé{tripsByStatus.arrived > 1 ? 's' : ''}
                            </span>
                          )}
                          {tripsByStatus.closed > 0 && (
                            <span className="px-2 py-1 text-xs bg-blue-100 text-blue-700 rounded">
                              {tripsByStatus.closed} Fermé{tripsByStatus.closed > 1 ? 's' : ''}
                            </span>
                          )}
                        </div>
                      </div>
      ...
      {viewMode === 'table' && waves.length > 0 && (
        <DataTable
          ...
          pagination={true}
          itemsPerPage={10}
          emptyMessage="Aucune vague clôturée trouvée"
        />
      )}
    </div>
  );
...
export default ExpressWavesHistory;
