...
import { businessWaveService, businessConvoyService } from '../../services/api/business';
...
const WaveDetail = () => {
  ...
  const loadWave = async () => {
    try {
      const response = await businessWaveService.getById(id);
  ...
  const loadConvoys = async () => {
    try {
      setLoading(true);
      const response = await businessConvoyService.getAll({ business_wave_id: id });
      if (response.data.success) {
        const responseData = response.data.data;
        if (Array.isArray(responseData)) {
          setConvoys(responseData);
        } else if (responseData?.data && Array.isArray(responseData.data)) {
          setConvoys(responseData.data);
        } else {
          setConvoys([]);
        }
      }
  ...
  const handleEdit = (convoy) => {
    ...
    setFormData({
      name: convoy.name || '',
      traveler_name: convoy.traveler_name || '',
      from_country: convoy.from_country || '',
      from_city: convoy.from_city || '',
      to_country: convoy.to_country || '',
      to_city: convoy.to_city || '',
      planned_departure_date: convoy.planned_departure_date ? convoy.planned_departure_date.split('T')[0] : '',
      planned_arrival_date: convoy.planned_arrival_date ? convoy.planned_arrival_date.split('T')[0] : '',
      actual_departure_date: convoy.actual_departure_date ? convoy.actual_departure_date.replace('T', 'T').substring(0, 16) : '',
      actual_arrival_date: convoy.actual_arrival_date ? convoy.actual_arrival_date.replace('T', 'T').substring(0, 16) : '',
      status: convoy.status || 'planned',
      notes: convoy.notes || '',
    });
    setErrors({});
    setIsModalOpen(true);
  ...
  const confirmDelete = async () => {
    try {
      await businessConvoyService.delete(selectedConvoy.id);
      await loadConvoys();
  ...
  const handleSave = async () => {
    try {
      ...
      if (selectedConvoy) {
        // Modifier
        response = await businessConvoyService.update(selectedConvoy.id, dataToSend);
      } else {
        // Créer
        response = await businessConvoyService.create(dataToSend);
      }
  ...
  const columns = [
    {
      header: 'Nom',
      accessor: (convoy) => convoy.name,
      sortable: true,
    },
    {
      header: 'Voyageur',
      accessor: (convoy) => convoy.traveler_name,
      sortable: true,
    },
    {
      header: 'De',
      accessor: (convoy) => `${convoy.from_city}, ${convoy.from_country}`,
      render: (convoy) => (
        <span className="text-sm text-gray-900">{convoy.from_city}, {convoy.from_country}</span>
      ),
      sortable: false,
    },
    {
      header: 'Vers',
      accessor: (convoy) => `${convoy.to_city}, ${convoy.to_country}`,
      render: (convoy) => (
        <span className="text-sm text-gray-900">{convoy.to_city}, {convoy.to_country}</span>
      ),
      sortable: false,
    },
    {
      header: 'Départ prévu',
      accessor: (convoy) => convoy.planned_departure_date,
      format: 'date',
      sortable: true,
    },
    {
      header: 'Date de fin',
      accessor: (convoy) => convoy.end_date,
      format: 'date',
      render: (convoy) => convoy.end_date ? formatDate(convoy.end_date) : '-',
      sortable: true,
    },
    {
      header: 'Statut',
      accessor: (convoy) => convoy.status,
      format: 'status',
      render: (convoy) => (
        <span className={`px-2 py-1 text-xs font-medium rounded-full ${getStatusClass(convoy.status)}`}>
    ...
    {
      header: 'Commandes',
      accessor: (convoy) => convoy.orders_count || 0,
      render: (convoy) => (
        <span className="text-sm font-medium text-gray-900">{convoy.orders_count || 0}</span>
      ),
      sortable: true,
    },
  ];
  ...
  const calculateProfitability = () => {
    ...
    // Revenus en CFA : somme de tous les montants convertis en CFA
    const totalRevenueCFA = wave.orders?.reduce((sum, order) => {
      const amount = parseFloat(order.total_amount) || 0;
      // Convertir en CFA selon la devise d'enregistrement
      if (order.currency === 'CFA') {
        return sum + amount;
    ...
    // Revenus en MAD : somme de tous les montants convertis en MAD
    const totalRevenueMAD = wave.orders?.reduce((sum, order) => {
      const amount = parseFloat(order.total_amount) || 0;
      // Convertir en MAD selon la devise d'enregistrement
      if (order.currency === 'MAD') {
        return sum + amount;
      } else if (order.currency === 'CFA') {
        return sum + (amount / exchangeRate);
      }
      return sum;
    }, 0) || 0;
    ...
    const convoyCostsCFA = convoys.reduce((sum, convoy) => {
      const convoyCost = convoy.costs?.reduce((s, cost) => {
        const amount = parseFloat(cost.amount) || 0;
        // Convertir en CFA si la devise est MAD
        if (cost.currency === 'MAD') {
    ...
    return {
      ...
      convoyCostsMAD: convoyCostsMAD,
      convoyCostsCFA: convoyCostsCFA,
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-gradient-to-r from-orange-600 to-orange-700 dark:from-orange-700 dark:to-orange-800 rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-lg">
        <div>
          ...
          <p className="text-orange-100 dark:text-orange-200 text-sm sm:text-base mt-1">Code: {wave.code} | {formatDate(wave.start_date)} - {wave.end_date ? formatDate(wave.end_date) : 'En cours'}</p>
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center space-y-2 sm:space-y-0 sm:space-x-3 w-full sm:w-auto">
          ...
          <div className="flex items-center space-x-1 bg-white/20 backdrop-blur-sm rounded-xl p-1">
            <button
              ...
              className={`px-3 py-2 rounded-lg transition-all duration-200 ${viewMode === 'cards' ? 'bg-white dark:bg-gray-800 text-orange-600 dark:text-orange-400 shadow-md' : 'text-white hover:bg-white/10'}`}
            >
            ...
            <button
              ...
              className={`px-3 py-2 rounded-lg transition-all duration-200 ${viewMode === 'table' ? 'bg-white dark:bg-gray-800 text-orange-600 dark:text-orange-400 shadow-md' : 'text-white hover:bg-white/10'}`}
            >
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {profitability && hasAnyRole(['boss', 'admin']) && (
        <div className="bg-gradient-to-r from-orange-50 to-orange-100 dark:from-orange-900/20 dark:to-orange-800/20 rounded-xl shadow-lg p-4 sm:p-6 border border-gray-100">
          <div className="flex items-center space-x-2 mb-4 sm:mb-6">
            <div className="h-1 w-6 sm:w-8 bg-gradient-to-r from-orange-600 to-orange-700 rounded-full"></div>
      ...
      {viewMode === 'cards' && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {loading ? (
            ...
            convoys.map((convoy) => (
              <div key={convoy.id} className="bg-white rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 p-4 sm:p-6 border border-gray-100 transform hover:-translate-y-1">
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <h3 className="text-lg font-semibold text-gray-900">{convoy.name}</h3>
                    <p className="text-sm text-gray-500">{convoy.traveler_name}</p>
                  </div>
                  <span className={`px-2 py-1 text-xs font-medium rounded-full ${getStatusClass(convoy.status)}`}>
                    {formatStatus(convoy.status)}
                  </span>
                ...
                <div className="space-y-3 mb-4">
                  <div className="flex items-center text-sm text-gray-600">
                    ...
                    <span>{convoy.from_city}, {convoy.from_country} → {convoy.to_city}, {convoy.to_country}</span>
                  </div>
                  ...
                  <div className="flex items-center text-sm text-gray-600">
                    ...
                    <span>Départ: {formatDate(convoy.planned_departure_date)}</span>
                  </div>
                  ...
                  {convoy.end_date && (
                    <div className="flex items-center text-sm text-gray-600">
                      ...
                      <span>Date de fin: {formatDate(convoy.end_date)}</span>
                    </div>
                  )}
                  ...
                  <div className="pt-3 border-t border-gray-200">
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className="font-semibold text-gray-900">{convoy.orders_count || 0}</span>
                    </div>
                  </div>
                </div>
      ...
      {/* Modal pour créer/éditer un convoi */}
      <Modal
        isOpen={isModalOpen}
        onClose={() => {
          setIsModalOpen(false);
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <FormInput
              ...
              onChange={(e) => setFormData({ ...formData, planned_departure_date: e.target.value })}
              error={errors.planned_departure_date?.[0]}
              required
            />
            <FormInput
              ...
              error={errors.planned_arrival_date?.[0]}
              min={formData.planned_departure_date}
            />
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <FormInput
              ...
              error={errors.actual_departure_date?.[0]}
            />
            <FormInput
              ...
              error={errors.actual_arrival_date?.[0]}
            />
          </div>
          ...
          <div className="flex justify-end space-x-3 pt-4 border-t border-gray-200">
            ...
            >
              {saving ? (
                ...
                <span>{selectedConvoy ? 'Modifier' : 'Créer'}</span>
              )}
            </button>
          </div>
        </div>
